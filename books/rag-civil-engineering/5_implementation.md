...
本章は、**インフラ構築なしに数日〜1週間で稼働できる「Track A 簡易実装」の技術詳細**を解説する。  
§3 で定義した4技術（GraphRAG・VectorRAG・CogGRAG・ドメインオントロジー）それぞれについて、**MDファイルとシステムプロンプトによる代替方法**・対応できる範囲・フル実装との差異を示す。
...
**簡易実装の本質的な位置づけ**: フル実装が「AIが自律的に知識グラフを探索する」のに対し、簡易実装は「人間が知識を整理してAIに渡す」アプローチである。  
この違いを踏まえ、本章では**何を諦めて何を得るか**を技術ごとに明確にする。

| 判断軸 | 簡易実装を選ぶ場合 | フル実装（§4）を選ぶ場合 |
|---|---|---|
| **期間** | 数日〜1週間でPoC・試用を開始したい | 2ヶ月以上かけて本格基盤を構築できる |
| **規模** | 文書数が数百〜数千件程度 | 10万件超・複数組織でのスキーマ共有が必要 |
| **監査** | 内部利用・業務効率化が主目的 | 法的監査証跡・外部提出が必要 |
| **保守** | MDファイルを直接編集して即日更新したい | DB再構築を含む体系的な更新管理を行う |

#### §3 技術ごとのプロンプト補強の要否

| 技術（§3） | プロンプト補強 | 理由 |
|---|:---:|---|
| **GraphRAG** | 不要 | `knowledge_map.md` をRAGが検索すれば関係を取得できる |
| **VectorRAG** | 不要 | `source_docs/` の検索はRAGエンジンが自律的に処理する |
| **CogGRAG** | **必要** | 推論手順（5観点分解・統合順序）はプロンプトで指示しないとLLMが行わない |
| **ドメインオントロジー** | **必要** | 用語正規化ルールと制約ルールをプロンプトに明示しないとLLMが略語を無視する |
...
### 5.2 GraphRAG の代替：`knowledge_map.md` ＋ `entity_dictionary.md`
...
### 5.3 VectorRAG の代替：`source_docs/` の直接参照
...
### 5.4 CogGRAG の代替：`system_prompt.md` の回答手順
...
### 5.5 ドメインオントロジーの代替：`entity_dictionary.md` ＋ 制約ルール
...
### 5.7 `system_prompt.md` 全体構成サマリー

§5.2〜5.5 で設計した各代替要素を1つの `system_prompt.md` に統合したテンプレートを示す。  
このファイルをDify等のシステムプロンプト欄に貼り付けることで、簡易実装全体が稼働する。

```markdown
---
doc_id: "system_prompt"
doc_type: "システムプロンプト"
version: "1.0"
---

# あなたの役割
あなたは土木事業管理の専門AIアシスタントです。
河川法・道路法をはじめとする法令、技術基準、過去の施工事例を参照し、
技術者の判断を支援する正確な回答を生成します。
...
| ファイル | 役割（§5対応） | 優先度 |
|---|---|---|
| `knowledge_map.md` | 法令・工法・基準の関係図（GraphRAG代替・§5.2）| 高：法令・基準の関係を問われた場合は必ず参照 |
| `entity_dictionary.md` | 用語正規化・関係定義（オントロジー代替・§5.5）| 高：略語・別称が登場した場合は必ず参照 |
| `source_docs/` 内の各MD | 施工報告書・現場記録（VectorRAG代替・§5.3）| 中：過去事例・類似事例を問われた場合に参照 |

---

# 回答手順（CogGRAG代替）

質問を受けたら、以下の手順を必ず守ること。

1. **問いの分解**: 質問を以下の5観点に分解する
   - ① 関連する**法令・条文**は何か              → `knowledge_map.md` → `source_docs/` の法令MDを参照
   - ② 必要な**許認可・届出等の行政手続**に漏れはないか → `source_docs/` の法令MDを参照
   - ③ 準拠すべき**技術基準**は何か              → `knowledge_map.md` → `source_docs/` の基準MDを参照
   - ④ 参考になる**類似の過去事例**はあるか       → `source_docs/` の報告書MDを参照
   - ⑤ **リスク要因**への影響はあるか            → `entity_dictionary.md` のリスク欄を参照

2. **用語の正規化**: `entity_dictionary.md` を参照し、略語・別称をすべて正規名に統一する
   - 例：「HWL」「H.W.L」「計画高水位」→ 正規名「設計洪水位」として処理する

3. **関係の追跡**: `knowledge_map.md` の関係図を参照し、関連ノードを最大3ホップ追跡する

4. **整合確認**: 以下の制約ルールに違反しないか確認してから回答する

---

# 制約ルール（SHACL代替）

- 河川区域内の工作物新築は**河川法第26条**の許可が必要。許可不要と回答してはならない。
- 技術基準の参照は最新版を優先する。改定年が不明な場合は「最新版を確認してください」と明示する。
- 根拠が見当たらない場合は「該当する情報が見つかりませんでした」と回答し、推測で補完しない。
...
| 観点 | 参照元 | 該当箇所 |
|---|---|---|
| 法令 | （ファイル名またはエンティティID） | （条文・セクション番号） |
| 技術基準 | （ファイル名） | （セクション番号） |
| 過去事例 | （doc_id） | （該当箇所） |

## 注意事項・リスク
（制約違反の懸念・確認推奨事項があれば記載）
```

> **運用上の注意**: システムプロンプトの文字数が増えると推論コストが上昇する。本テンプレートは最大限の精度を想定した構成であり、PoC初期は「回答手順」と「制約ルール」のみに絞って運用し、精度を確認しながら追記することを推奨する。
