...
土木事業管理における知識・判断継承のためのRAG_Sysyem構築提案書（素案）
...
本提案は、土木事業管理における「ベテラン技術者の暗黙知・判断ロジックの消失」という構造的課題に対し、4つの先進技術を統合したRAG_Sysyemで解決することを目指すものである。
...
| トラック | 開発スタイル | アプローチ | 期間 | 対象 |
|---|---|---|---|---|
| **Track A（簡易）** | **ノーコード〜ローコード**<br>M365 Copilot Agent Builder / Dify | MD＋システムプロンプトで近似実現（§5-B） | 数日〜1週間 | PoC・試用・小規模運用 |
| **Track A+C（★主力推奨）** | **ローコード**<br>M365 Copilot Studio / Dify | MD＋マルチエージェント監理構造（§5-B/C） | 1〜2週間 | **大多数の実務用途に対応。AIの進歩を直接享受** |
| **Track B（本格）** | **フルコード（プロコード）** | 7ステップ＋インフラ構築による完全実装（§4） | 2ヶ月〜 | 法的監査証跡・10万件超・複数組織スキーマ共有 |
