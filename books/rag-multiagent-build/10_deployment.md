...
- [ ] Dify / AutoGen サービスをプライベートネットワーク内に配置
...
## 10.3 Track B: AutoGen 本番構成（例）

```
[Nginx / Azure App Gateway]
        ↓
[FastAPI サーバー]  ←── [AutoGen パイプライン（asyncio）]
        ↓                        ↓
[PostgreSQL / Redis]      [pgvector / Chroma (VectorDB)]
        ↓
...
| AutoGen パイプライン定義 | Git タグ（`v1.0`, `v1.1`） |
...
| **本番移行** | 1ヶ月 | Track B（AutoGen）に移行 or Dify 本番強化、全部門展開 |
