...
*   **アーキテクチャのポイント (Multi-LoRA):**
    最新の推論サーバーは、1つの重い基盤モデルをメモリに常駐させたまま、リクエストごとに軽量なLoRAアダプター（法務用、技術用など）を**動的に切り替えて**推論する機能（Multi-LoRA Serving）を持っています。これにより、GPUメモリを節約しつつ、多数の専門家モデルを同時に運用できます。
*   **技術スタック:**
