...
- [プロジェクトCDE — 構想・機能・運用ガイド](#プロジェクトcde--構想機能運用ガイド)
  ...
  - [6. SSOTデータ保存先（ストレージ）の選定](#6-ssotデータ保存先ストレージの選定)
    ...
    - [6.7 CDEの決済フロー（WIP → Shared → Published / Archived）](#67-cdeの決済フローwip--shared--published--archived)
...
- **データの権威性**: タスクチーム（各専門チームの作業環境・各チーム裁量）から成果物を**CDEに提出**し、WIP（作業中・チーム間調整待ち）→ Shared（共有中・レビュー中）→ Published（承認済・正本）のステータス管理で、業務上の判断・承認をCDE内で完結させる。**現実はタスクチームからすべてが始まる**が、その作業環境（ローカルPC・自社サーバ等）はCDEの外側であり、**WIPへの提出が正式CDE管理の入口**となる
...
| SSOT保証 | ISO 19650準拠の **WIP（作業中）→ Shared（共有中）→ Published（承認済・正本）** のステータス管理により、共有・公開されるデータが常に唯一の正本であることを保証する。現実は各専門チーム（タスクチーム）が自己の作業環境で成果物を準備し、**WIPへの提出をもって正式CDE管理が始まる**。タスクチームの作業環境（ローカルPC・自社サーバ等）はCDEストレージの対象外 |
...
**フォルダ階層の設計方針：ステータス（WIP / Shared / Published / Archived）を最上位に置く理由**

CDE のステータスは「誰が見られるか・変更できるか」＝**公開対象者と変更可否の変化**を表す：

| ステータス | 公開対象 | 変更 | 用途 |
|-----------|---------|------|------|
| **TaskTeam** | **当該チームのみ（CDE外）** | **可** | **草稿・内部調整・WIP提出前の作業** |
| WIP | 当該タスクチームのみ | 可 | 正式CDE管理の入口・作業・レビュー・指摘対応 |
| Shared | プロジェクト関係者全員 | 不可 | 発注者による審査・承認待ち（契約上の受理） |
| Published | 関係機関・外部関係者 | 不可 | 関係機関等への公開版・現行正本 |
| Archived | 全関係者 | 不可（確定・固定） | 業務・フェーズ完了単位の報告書・納品物の保管 |

`TaskTeam` は厳密にはCDE外（各チームのローカルPC・NAS等）だが、CDE上に置く場合は**当該チームのみアクセス可の作業フォルダ**として機能する。草稿がまとまった段階で `WIP/` へ提出することで正式CDE管理が開始される。

業務フォルダを最上位にすると `02_設計/WIP`・`03_関係機関協議/WIP`… と**フォルダ数 × 5 回**の権限設定が必要になり、設定漏れ・担当変更時のメンテナンスコストが増大する。ステータスを最上位にすれば `Shared/`・`Published/`・`Archived/` への権限付与が**各1回**で全業務フォルダに継承される。ISO 19650 のCDE概念（ステータスが情報管理の主軸）にも忠実であることから、**ステータス → 業務** の順を採用する。
...
│   │              WIPで承認完了した成果物。ここかPublished（公開）とArchived（確定保管）に並行分岐
│   ├── 📁 01_事業全体計画の整理
│   ├── 📁 02_測量・調査・設計業務等の指導・調整等
│   ├── 📁 03_地元及び関係行政機関等との協議
│   ├── 📁 04_事業管理
...
│   │              業務・フェーズ完了単位の報告書・納品物・承認済み議事録等
│   │              ※Published（現行公開版）と並立して「完了分」を蓄積する場所
│   ├── 📁 01_事業全体計画の整理
│   ├── 📁 02_測量・調査・設計業務等の指導・調整等
│   ├── 📁 03_地元及び関係行政機関等との協議
│   ├── 📁 04_事業管理
...
### 6.7 CDEの決済フロー（WIP → Shared → Published / Archived）
...
正しい使い分け：
- **TaskTeam**: 成果物の作成・修正・再提出（CDE外）
- **WIP**: 共有依頼前の受注者内確認（読取のみ・変更禁止）
- **Shared**: 発注者による審査・承認待ち（契約上の受理）
- **Published**: 承認済み現行版・関係機関等への公開
- **Archived**: 業務・フェーズ完了単位の報告書・納品物・承認済み議事録の保管
