...
**測量・設計・監理の役割分担（事業管理視点）**

| 業務 | 主データ形式 | 管理ツール・方針 |
|------|------------|---------------|
| 測量（現況取得） | 点群（LAS/LAZ）・写真 | CDEの原本として保管。Potree等で共有・比較表示 |
...
> ※4 SharePointのKO-Aは○：Azure AD B2Bゲスト招待は相手方Microsoftアカウントまたは相手方組織のゲスト許可が前提。許認可機関・行政機関では制限されるケースが多い。KO-B（国内DC）は◎のため、**関係機関がゲスト招待を許可できることを事前確認できれば✅利用可**。  
...
| 状況 | 推奨 | 根拠 |
|------|------|------|
| **国・自治体が発注者（ISMAP準拠必須）** | **SharePoint (Microsoft 365)** | ISMAP登録済み（C21-0013-2）。国内リージョンでデータ所在国要件を満たす。M365を使用する組織間での承認ワークフロー・共有ログを標準機能で実現できる。M365を持たない関係機関へはゲストリンク・パスワード付きURL共有で対応（相手方の操作習熟が課題） |
| **建設コンソーシアム・外部共有が多い** | **Box** | ISMAP登録済み（C21-0017-2）。①アカウント（コラボレータ）による公式情報共有と②共有リンク（パスワード付き）を使い分けられる。大容量ファイル（BIM/CIM・点群）対応。関係機関との協議・承認はアカウント共有が原則 |
| **ISMAP要件確認が取れない・調達が間に合わない** | **NAS + VPN（暫定）** | 庁内・事務所内に閉じた環境として暫定利用可。外部共有はVPN接続が必要で長期運用には適さない |

## 2.5 推奨フォルダ構成例（SharePoint / Box 共通）
...
4. アプリ／配信方式の選定（例: Revit/Navisworks は層1、Cesium/KOLC+ は層2、Re:Earth は層3）
...
- **共有方法**: アカウントベース（SSO/MFA）またはタイプA共有リンク（ID＋パスワード・有効期限・ダウンロード制御）。外部委託は短期トークンで用途限定。
...
|---|---|---|---|---:|---|
| 施工管理者（現場監督） | 層2 | アカウント（認証付き）/ 必要時タイプA | Trimble Connect, KOLC＋, Cesium | 要（組織アカウント） | 要確認（官公庁案件は国内DC/ISMAP要件） |
| 現場技術者（測量・出来形担当） | 層1＋層2 | アカウント（認証付き） | Potree（点群）、Navisworks, QGIS | 要（多くは有料ライセンス） | 要確認 |
| 下請け業者 | 層2 | タイプA または アカウント | Cesium / KOLC＋ / Re:Earth | 要（短期はリンクで無アカウント可） | 要確認 |
| BIMコーディネータ / モデラー | 層1 | アカウント（認証付き） | Revit / Navisworks / Bentley iTwin | 要（有料ライセンス） | 要確認 |
| 品質管理・監理（第三者） | 層2 | アカウント または タイプA | QGIS/Lizmap、Cesium | 要（組織アカウント） | 要確認 |
| 発注者・行政（窓口担当） | 層2＋層3（広報用抜粋） | アカウント／公開はタイプBで抜粋 | ArcGIS Online、Re:Earth | 要（多くはアカウント提供） | KO: 国内DC/ISMAP等遵守が必須 |
...
| **NAS ＋ SharePoint/Box同期**（〔B〕と同構成） | BIM/CIM担当・ACCライセンスなし等 | ◎ | ◎ | ◎ | ◎（※a） | ○ | **37** |
...
> ※f ACCはAutodesk専用プラットフォームであり、Box・SharePoint等の汎用クラウドストレージとは別物。BIM/CIM作業ファイルを汎用クラウドストレージに直接保存・同期することは非推奨または動作保証外。CDEをBox/SharePointとする場合、BIM/CIMチームの作業ファイルはACC内に留まり、完成成果物（IFC・PDF・点群等）をCDE（Box/SharePoint）のWIPへエクスポート提出するワークフローが基本。
...
|--------|:-------:|---------|-----------|:---:|
| **Cesium / 3D Tiles** | 層2/3 | 3DTILES/glTF対応、自己ホスト可／Cesium ion SaaS | ionトークン、カスタムSSO/APIトークン | Cesium ionは海外SaaS。自己ホストで国内運用可 |
| **KOLC＋** | 層2 | 3Dタイル・点群・4D・注記・ワークフロー | SAML2.0（SSO）・2段階認証 | 国内DC（さくら/AWS東京）・ISMAP登録・ISO27001 |
| **CIMPHONY Plus** | 層2 | 点群統合・注記・計測・断面出力 | SSO/法人向け連携 | NETIS登録済み |
| **Re:Earth** | 層2/3 | CesiumベースのCMS・ストーリーテリング、自己ホスト可 | REST/GraphQL API、プラグインで認証連携可 | 日本事業者。ISMS/国内DC要確認 |
| **torinome** | 層2/3 | 軽量ビューワー・タイル配信 | 簡易認証（追加の認証レイヤ推奨） | 自己ホスト前提 |
| **Potree** | 層1/2 | 点群ビューワー・タイル化ツール | 認証機能なし（リバースプロキシ/WAFで保護が必要） | 自己ホストで国内運用可 |
| **iTowns** | 層2/3 | 地理空間3D表示・タイル対応 | カスタム実装でSSO連携可 | 自己ホスト |
| **Autodesk (Forma / Construction Cloud)** | 層1/2 | BIM表示・注記・4D/管理機能 | SSO/OAuth対応（商用SaaS） | グローバルSaaS。国内DC/ガバメント向けは契約確認 |
| **Bentley iTwin** | 層1/2 | IFC・BIM・デジタルツイン統合 | OAuth2 / SSO、エンタープライズ向け | データセンター位置は要確認 |
| **Trimble Connect** | 層1/2 | BIM点群データ共有・管理 | SSO/API対応 | 要確認 |
| **QGIS / QGIS Server** | 層1/2/3 | デスクトップGIS・豊富な解析機能・プラグイン | LDAP/SSO連携可 | 自己ホストで国内運用可 |
| **ArcGIS Pro** | 層1/2 | 高度な解析・編集機能 | SSO/SAML対応（Esri） | ArcGIS Enterprise でセルフホスト可 |
| **Lizmap** | 層2/3 | QGISによる地図をWeb公開 | LDAP等の認証連携・グループ管理可 | 自己ホスト可 |
| **ArcGIS Online** | 層2/3 | Webマップ・ダッシュボード・ストーリーマップ | ArcGIS組織アカウント／SAML | グローバルSaaS。国内要件は契約確認 |

> 注意: 各アプリの「認証方式（SAML/OAuth/独自）」「APIの有無」「データ所在（国内DC要件）」は導入前に必ず確認する。
