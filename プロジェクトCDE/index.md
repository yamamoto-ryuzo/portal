...
**受信・通知方針**：外部受領は原則としてBoxのFile Requestを使用し、必要に応じてメールアドレス入力を必須化するか認証付きポータルを併用してアップローダーを識別する。File Request作成時はフォルダコラボレータへのメール通知を有効にし、Box WebhookまたはPower Automateで「新規アップロード検知→（ウイルス/コンテンツ）スキャン→管理者・担当チームへメール/Teams通知」を自動化して見落としを防ぐ。通知先は管理者と担当チームの最低2箇所を設定し、導入時にテスト通知を実施、通知テンプレートと障害時連絡手順を運用ルールに含める。
