...
|---|---|---|---|---|
| TaskTeam 内部レビュー（草稿合意） | 受注者チーム内 | タスク管理ツール（Backlog 等） | P1: タスク管理ツール + 外部APIワーカー（標準） | 日常業務に近く、担当・期限・差戻しを管理しやすい |
| TaskTeam -> WIP 提出可否 | 受注者側責任者 | タスク管理ツール + 外部APIワーカー | P1: タスク管理ツール + 外部APIワーカー（推奨） | 承認イベントを Webhook 起点にして、WIP への登録処理を自動化しやすい |
| WIP -> Shared 共有依頼 | 受注者側責任者 | タスク管理ツール + 外部APIワーカー（必須） | P1: タスク管理ツール + 外部APIワーカー（推奨） | 監査で重要な遷移。task_id とファイルIDの紐付けを残しやすい |
| Shared -> Published 正式承認 | 発注者（監督職員等） | **P1+P2 ハイブリッド**（タスク管理ツール + 独自承認UI）を強く推奨。短期は P1 単体で代替可。 | **P1+P2: タスク管理ツール + 独自承認UI（推奨）**<br>短期代替: P1 単体 | 発注者担当者が直接操作する正式承認フロー。承認UI品質が低いと運用が破綻するため、UI/UXは事実上のKO条件。P4〜P6 の汎用UIは本フローへの適用は原則非推奨。 |
| Shared -> Archived 保管判断 | 発注者/PMO | CDEネイティブ運用 + バッチワーカー | P5: ローコード + 外部APIワーカー（Box Relay系） または P4: ローコード + 外部APIワーカー（Power Automate系） | 公開とは別の保管判断。定期処理・保管ルール適用が中心 |
| 緊急差替え・取り下げ | 発注者責任者 + システム管理者 | 手動承認 + 外部APIワーカー（二重承認） | P4〜P6: ローコード + 外部APIワーカーの運用拡張 | 例外処理は自動化しすぎず、権限者の明示承認を優先 |
...
|---:|---|---|---|---|---|---|
| **1** | **P1+P2** | **【推奨ハイブリッド】タスク管理ツール + 独自承認UI** | Backlog + FastAPI ワーカー + React/Next.js 承認UI (Coolify) | **UI**: プロジェクト専用の独自承認画面（P2 で構築）<br> **トリガ**: 独自UI からの API 呼び出し または Backlog Webhook<br> **処理**: FastAPI 等の外部APIワーカーが Box 操作・監査ログ・バッチを実行します。 | 監査証跡（P1）と承認UI品質（P2）を両立。発注者担当者が直接操作する `Shared→Published` に最適。 | 初期開発コストが最大。PoC は P1 単体から開始し、承認UI課題が顕在化した段階で P2 独自UI を追加するロードマップを推奨。 |
| 2 | P1 | タスク管理ツール + 外部APIワーカー | Backlog + 外部APIワーカー（FastAPI等） | UI: Backlog（タスク完了/承認）<br> トリガ: Webhook（起点）<br> 処理: FastAPI 等の外部APIワーカーが enqueue → Box 操作・権限変更・大容量バッチを実行します。承認UIはタスク管理側または外部ローコードで提供。 | 既存のタスク運用をそのまま活用でき、導入が速い。PoC 開始に最適。 | タスクとファイルの紐付け設計、Webhook 署名検証、冪等性・キュー管理等の実装が必要。発注者担当者向け承認UIは汎用的なため、本番移行時は P1+P2 への移行を検討。 |
| 3 | P2 | フルスタック | Coolify | UI: Coolify 上の React フロントで承認UIを提供<br> 処理: 同一環境の API ワーカーで Box 操作・監査ログ・バッチ処理を実行します。 | UX と柔軟性が最大。フロント・API・ワークフローを同一環境で統合可能。独自UI単体での構築に向く。 | バックエンドの監査証跡は P1 より弱いため、P1+P2 ハイブリッドが理想。セルフホストの監視・バックアップ・運用負荷が必要。 |
| 4 | P3 | Serverless / Managed | Vercel | UI: Vercel のフロント/Serverless 関数で簡易 UI を提供<br> 処理: Serverless 関数または外部ワーカーで Box 呼び出し・キュー投入を実行します（必要に応じ Cloud Tasks 等と併用）。 | マネージドでスケーラブル、フロントと関数の統合が容易。 | 監査証跡・トレーサビリティが P1 に比べ弱い。公共事業での証跡要件を別途設計する必要あり。マネージド制約（コールドスタート・実行時間等）に注意。 |
...
| 7 | P6 | **【KO-U】** ローコード + 外部APIワーカー | n8n + FastAPI | UI: n8n のワークフロー/承認UI（ローコード）; 処理: 外部APIワーカー（FastAPI等）で重い Box 操作・バッチ処理・監査ログを実行します。 | 自ホストで柔軟に連携可能。受注者内部・PoC 限定なら有効。 | n8n の承認UIは開発者向けで一般業務担当者には事実上使用不可。**発注者向け承認フローへの適用は KO-U**。受注者内部フロー・PoC に限定して使用してください。 |
...
- 評価基準（重み）: 監査性(Auditability) 50、セキュリティ/コンプライアンス 35、インフラ・保守運用 10、導入コスト 5（合計100）。
...
|---|:---:|---|---|---|
| **監査性** (Auditability) | 50 | 操作ログ・承認証跡・差戻し履歴が外部DB等に永続保存され、task_id/file_id 等のキーでタスク↔ファイル↔承認イベントを横断追跡・検索・証跡出力できるか（トレーサビリティを含む） | 操作イベントを構造化ログとして外部DBに保存、task_id/user_id/timestamp で横断検索・CSV出力可。タスク管理ツールが task_id を Webhook ペイロードで自動提供し追加設計なしに紐付け可能 | ログが標準機能の範囲のみで後から検索・出力できない、またはタスク↔ファイルの紐付けが手動運用・IDが引き継がれない |
| **セキュリティ/コンプライアンス** | 35 | シークレット管理・認証方式・Webhook署名検証・最小権限・通信暗号化が適切か | Vault等でシークレット一元管理、サービスアカウント最小権限、署名検証・冪等性実装済み | シークレットがコードや環境変数に平文保存、署名検証なし、権限が過剰 |
| **インフラ・保守運用** | 10 | IT管理者・運用担当者がシステムを維持するコスト。サーバ/コンテナの監視・パッチ適用・バックアップ・障害対応等の運用負荷 | フルマネージド、監視・スケーリング・パッチが自動、運用担当者の作業量が少ない | セルフホスト・24時間監視が必要、パッチ・バックアップ・障害対応を手動で行う必要あり |
| **導入コスト** | 5 | 初期構築・設定・ライセンス取得・PoC開始までの時間・費用 | 既存ツールの設定変更のみ、追加費用ゼロ、1週間以内に PoC 開始可 | カスタム開発が必要、新規サービス契約・インフラ構築・社内審査に数ヶ月を要する |
...
| パターン番号 | パターン | KO-U | 得点 (100) | 監査性(50) | セキュリティ(35) | インフラ・保守運用(10) | 導入コスト(5) | 根拠（要約） |
|---|---|:---:|---:|---:|---:|---:|---:|---|
| **P1+P2** | **【推奨ハイブリッド】タスク管理ツール + 独自承認UI** | ✅ 適合 | **92** | **50** | **35** | **5** | **2** | task_id による監査証跡・横断追跡（P1）と専用設計の承認UI（P2）を組み合わせ。監査性最高水準（50点）。**KO-U を確実にクリア**。ただし開発・保守コストは最大。 |
| P1 | タスク管理ツール + 外部APIワーカー (Backlog + FastAPI) | ⚠️ 要確認 | **87** | 40 | 35 | 8 | 4 | 既存タスク運用を活用、task_id による監査ログ・横断追跡が可能（40点）。システム品質は高いが **KO-U は利用者属性次第（7.2.3 C1 確認要）**。発注者担当者が直接操作する承認は P1+P2 への移行を検討。 |
| P2 | フルスタック (Coolify) | ✅ 適合 | **68** | 35 | 25 | 5 | 3 | 独自UIを完全構築可でKO-U確実にクリア。監査証跡・横断追跡は設計次第（35点）。P1+P2 ハイブリッドが理想。 |
| P3 | Serverless / Managed (Vercel) | ✅ 適合 | **66** | 30 | 25 | 8 | 3 | 独自UI構築可でKO-U確実にクリア。マネージドでインフラ保守が軽い（8点）。監査証跡は設計次第（30点）。 |
| P4 | ローコード + 外部APIワーカー (Power Automate + Azure Functions) | ❌ **KO-U** | ~~86~~ | 40 | 35 | 8 | 3 | **`Shared→Published` 適用不可**。バックエンドの監査・セキュリティは高水準だが承認UIが KO-U。受注者内部フローに限定。 |
| P5 | ローコード + 外部APIワーカー (Box Relay + FastAPI) | ❌ **KO-U** | ~~81~~ | 40 | 30 | 8 | 3 | **`Shared→Published` 適用不可**。バックエンドは良好だが承認UIが KO-U。受注者内部・WIP系フローに限定。 |
| P6 | ローコード + 外部APIワーカー (n8n + FastAPI) | ❌ **KO-U** | ~~69~~ | 35 | 25 | 6 | 3 | **発注者向け承認フロー適用不可**。承認UIが KO-U。PoC・受注者内部フロー限定。 |

> **KO 凡例**: ✅ 適合 = KO-U をクリア / ⚠️ 要確認 = 利用者属性（7.2.3 C1）による / ❌ KO = 発注者向け正式承認フローへの適用不可（得点は取り消し線）

推奨（事業監理観点）: 本番運用では **P1+P2 ハイブリッド**を目標に据え、まず **P1 単体**で PoC を実施し、承認UI の課題が顕在化した段階で P2 の独自承認画面を追加するロードマップが最も現実的です。P4〜P6 単体は「受注者内部フロー（TaskTeam/WIP系）」に限定し、発注者が直接操作する `Shared → Published` 承認への適用は原則避けてください。
...
### 7.2.2 Webhook起点での実装方式（FastAPIはベストか）
...
### 7.2.3 ローコードUIの限界と独自UI検討指針

ご指摘のとおり、P4〜P6 の「ローコード + 外部APIワーカー」パターンの承認UIは、**エンドユーザー（監督職員・担当者等）が直接操作する場面では操作しにくいケースがある**という現実的な課題があります。

#### ローコードUIの限界（KO判定の根拠）

P4〜P6 の承認UIは「壊滅的」と評価されるケースが多く、**発注者担当者が直接操作する正式承認フロー（Shared → Published）への適用は不可（KO-U該当）**です。

| パターン | 標準UI | KO判定 | 限界・操作しにくくなる場面 |
|---|---|:---:|---|
...
> **前提**: `Shared → Published`（発注者正式承認）のフローについては、C1〜C7 の合否に関わらず **独自UI（P1+P2 ハイブリッド）を強く推奨**します。P4〜P6 の汎用UIは、発注者担当者が直接触れる正式承認フローでは事実上KO条件として機能します。以下のチェックシートは「受注者内部フロー（TaskTeam/WIP系）や簡易フローに標準UIを使うか否か」の判断に活用してください。

「独自UIが必要かどうか」は一般的にプロジェクトごとに判断が必要です。以下のチェックシートで合計点を算出してください。
...
| 合計点 | 判定 | 推奨アクション |
|:---:|---|---|
| 0〜2点 | 標準UIで十分 | P1（または受注者内部フロー限定で P4〜P6）の標準承認UIで運用開始。モニタリングを継続する。 |
| 3〜4点 | 標準UIで運用しつつ改善余地あり | まず標準UIで PoC を実施し、課題が顕在化した箇所のみ独自UI化を検討する。 |
| 5〜7点 | 重要フローのみ独自UI化を推奨 | Shared → Published（発注者正式承認）等の高頻度・高重要フローから着手する。 |
| 8点以上 | フル独自UI構築を推奨（P2/P3） | P2（Coolify/フルスタック）または P3（Vercel / Cloud Run）で専用承認ポータルを構築する。 |

> 参考: 公共インフラ系の大規模プロジェクト（複数工区・発注者～受注者間の正式承認が必要）では C1+C2+C3+C4 が揃うことが多く、合計7〜9点になるケースが一般的です。

#### 独自UI追加の現実的なアプローチ

独自UIを追加する際も、**処理バックエンドは外部APIワーカー（P1・P4〜P6 のワーカー部分）を流用**できます。UIだけを差し替えるイメージです。
...
承認者ブラウザ
  ↓ 操作（承認ボタン/差戻し/コメント入力）
独自承認UI（React/Next.js 等: P2/P3 で構築）
  ↓ API呼び出し
外部APIワーカー（FastAPI等: P1・P4〜P6 のワーカー部分と共通）
  ↓
Box 操作 / 権限変更 / 監査ログ保存
```

- **短期（PoC）**: まず P1 の標準UIで運用開始 → 使いにくさが顕在化した段階で独自UIを追加
- **中期（本番安定後）**: 独自UIを P2（Coolify）または P3（Vercel / Cloud Run）として構築し、バックエンドAPIは流用
- **独自UIが必要な承認のみ**: Shared → Published（発注者正式承認）などの重要フローだけ独自UI化し、他は標準UIのままでもよい

> ポイント: 最初から独自UIを作ろうとすると開発コストが高くなります。まず標準UIで PoC を動かし、実際の「使いにくさ」を確認してから必要最小限の画面だけ作るのが最も現実的です。
