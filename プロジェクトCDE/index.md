...
## 1.4. データ基盤 — アクセス層（Access Layer）と共有方法
本節ではアクセス層の三層モデルを示し、各層ごとに「共有方法」を最初に明示したうえで、認証・認可・配布手段・監査・運用上の要点を整理する。

### 三層モデル（概要）
- **目的:** どのデータを誰がどの方法で共有するかを層別に定義し、技術（認証・認可・監査・自動化）と運用（承認ゲート・有効期限・発行履歴）で強制する。
- **層1（設計編集）:** 高忠実度データの内部編集層（設計者・BIMコーディネータ等）。編集可・厳格な保護。
- **層2（関係者共有）:** 発注者・施工者・監理者など関係者向けの限定共有。参照・注記・レビュー中心。
- **層3（公開）:** 住民や一般向けの公開層。匿名化・軽量化した派生物のみ公開。

---

### 層1 — 設計編集（高保護・編集可）
- **共有方法:** CDE内のShared領域またはAPI経由の署名付き配布パイプライン。手動配布／メール添付は禁止され、承認済み派生物のみ配布。
- **認証:** SSO（OIDC/SAML）＋厳格MFA（ハードトークン/OTP）、短いセッション有効期限（例: 30分）。
- **認可:** RBAC＋細粒度ACL（オブジェクト／属性レベル）。最小権限で編集ロールを付与。
- **配布（技術）:** APIゲートウェイ経由、署名・ハッシュ付き派生物、バージョン管理。VPN/ゼロトラスト・IP制限を要求。
- **監査:** 変更ログ（誰が何を変更したか）を完全記録しSIEMへ集約。差分ロールバック対応。
...
- **配布（技術）:** 必要最小属性で派生物を生成、署名・ハッシュで整合性を担保。APIまたは制御付きリンクで配信。
- **監査:** アクセス／ダウンロードログを保持し定期レビュー。承認メタデータ（承認者・日時・派生物ID）を必須化。
- **運用:** 共有リンクは申請→承認→発行履歴を残す。期限切れ後は自動無効化。

### 層3 — 公開（低リスク・広報向け）
- **共有方法:** タイプB公開リンク（CDN配信）で公開。ただし公開前に完全匿名化・マスキング・解像度低減した派生物のみを公開し、公開期限と監視を設定する。
- **認証:** 閲覧は基本不要。公開設定変更は管理者のSSO権限（MFA）で保護。
- **認可:** 公開は事業管理の承認ゲート（Published）を通過した派生物のみ許可。
- **配布（技術）:** CDN＋WAF＋RateLimitで配信。公開派生物は署名・バージョン・元SSOT IDを持つ。
- **監査:** アクセス統計・監視ログで追跡。個人情報を含む公開は厳禁。公開物の根拠（元データID・承認記録）を保持。
- **運用:** 公開前にSSOTと照合し、公開後は定期チェックと期限管理。削除・訂正要求の対応フローを整備。

---

### 共通要件（全層）
- **認証基盤:** OIDC/SAML IdP、SCIM によるプロビジョニング。MFA を原則。
- **認可基盤:** RBAC／ABAC の実装、APIゲートウェイでスコープ管理。
- **通信:** TLS 1.2/1.3 による暗号化。
- **ログ/監査:** 全アクセス・操作ログをSIEMへ集約し最低90日保持（要件で延長）。
...
- フェーズ毎のアクセスポリシーと承認責任者を明記し、承認メタデータ（承認日時・承認者・派生物ID）を必須化する。

まとめ: 上記の三層モデルと共通要件を技術（認証・認可・ログ・パイプライン）および運用（承認ゲート・最小権限・監査）で一体的に運用することで、データの信頼性と公開の透明性を確保する。
...
## 3.6 推奨フォルダ構成例（SharePoint / Box 共通）
