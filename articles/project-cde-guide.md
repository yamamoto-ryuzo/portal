...
### データ種別ごとの処理と可視化パイプライン

調査・測量・設計で生成される主なデータ（`1.3.1` 記載）を前提に、可視化基盤での取り扱い方針と代表的な変換・配信パイプラインをまとめます。各データはCDEで原本を保管し、可視化用の「派生物」を自動生成して配信します。

- 調査データ（報告書・写真・ラスタ）
    - 代表フォーマット: PDF/Word（PDF/A 推奨）、JPEG/PNG、GeoTIFF
    - CDE保管: 原本はファイルストレージ（Box等）に保存、メタデータはPostGIS/メタデータDBに登録
    - 前処理: 画像→サムネ/オーバーレイ生成、GeoTIFFはタイル化（GDAL）、OCR/メタ抽出
    - 派生物: タイル化ラスタ（TMS/WMTS）、サムネ、GeoJSONメタ参照
    - 表示: GeoServer/QGIS Server → WMS/Tiled Map／Webアプリのドキュメントリンク表示

- 測量データ（座標・GNSS・点群・DSM）
    - 代表フォーマット: CSV/GPX/RINEX、LAS/LAZ/E57、GeoTIFF（DSM/DTM）、XYZ
    - CDE保管: 原本(LAS/E57/RINEX等) をアーカイブ、メタデータはJSONBで格納（EPSG・精度・取得日）
    - 前処理: PDAL/GDAL で投影・フィルタ（ノイズ除去）→ Entwine でタイル点群生成 → Potree/3DTiles 出力
    - 派生物: タイル点群（Entwine / Potree）、DEM/DSM タイル（GeoTIFF→TMS）、等高線ベクタ出力（GDAL）
    - 表示: Potree / Cesium（タイル点群） / WebGL ビューア

- 設計データ（図面・BIM・土木データ）
    - 代表フォーマット: DWG/DXF, IFC, Revit (.rvt), LandXML, OBJ/FBX
    - CDE保管: 元IFC/DWGをSSOTとして保管、属性メタ（IFC GUID / ssot_id）を紐付け
    - 前処理: IFC→glTF/3DTILES（IFC.js, ifcConvert, xeokit 等）／Revit→IFCエクスポート→3DTILES、CADはDXF→GeoPackage変換（必要時）
    - 派生物: 3DTILES / glTF（LOD・軽量化済）、ベクタタイル（Topological要約）
    - 表示: Cesium / IFC.js / 専用SaaSビューア（Trimble/Autodesk/Bentley）

- 共通処理・留意点
    - 座標系: すべての派生物にEPSGコードを明記し、投影誤差がないかチェックする
    - メタデータ: `ssot_id`、作成日、作成者、精度、処理履歴を必須化（GeoJSON properties / PostGIS JSONB）
    - タイリング/LOD: 点群・3Dモデルはタイル化＋LOD生成で配信性能を担保
    - 権限: 派生物はオリジナルのCDE権限ポリシーに従い公開設定（層1/層2/層3）を付与
    - ETL自動化: ファイル登録イベントをトリガにGDAL/PDAL/Entwine/IFC変換を自動実行し、CI（パイプライン）で派生物を再生成
    - 可視化参照リンク: 文書・図面はGIS属性として `file_url` を持たせ、GetFeatureInfo で即時参照可能にする

この節により、`1.3.1` のデータ一覧で示した各フォーマットが、可視化基盤においてどのように取り扱われるか（保管・前処理・派生物・表示）を明確にしました。
...
## 4.6 3D表示方針：中間フォーマットを `3DTILES` に統一

- **目的**: 本節は2D属性付与ではなく、3Dモデルの可視化フローとしての運用設計を示す。`3DTILES` を中間生成物の標準とし、取り込み→タイル化→配信→表示までを一貫して自動化する。
- **主要フロー（3D可視化視点）**:
    1. 取り込み：IFC/RVT/DWG, LAS/LAZ, 写真測量モデル 等を受け入れる
    2. 前処理：座標（EPSG/ECEF）整合、階層・セマンティクス抽出、マテリアル統合
    3. タイル化：メッシュ簡略化・LOD生成・テクスチャ統合・Draco/KTX2 圧縮を行い `3DTILES` を生成
    4. デプロイ：SuperMap/GISBOX 等の3DサーバまたはS3+CDNへデプロイしストリーミング配信を行う
    5. 表示：クライアント（Cesium/Three.js/Unity 等）はビュー依存でタイルを逐次取得してレンダリング
    6. インタラクション：断面・距離・属性クエリ・ハイライト等の3D操作を提供する

- **ARとの関係（可視化フロー内）**: ARは表示層の一端として扱う。原則は `3DTILES` を第一級の配信物とし、ARクライアントが非対応の場合はサーバ側で自動変換（glTF/GLB）して配信するが、変換は可視化フローの一部として自動実行する。
- **設計上の留意点**:
    - 3D固有のメタデータ（階層ID、素材、耐荷重等のセマンティクス）を `3DTILES` のタイルメタに保持して、クライアントからの3Dクエリで利用できるようにする。
    - ジオリファレンス情報を必ず保持し、クライアントはECEF→ローカル変換で表示位置を決定する。高さ基準（地表面 vs ジオイド）を明示する。
    - パフォーマンスはタイル粒度・LOD・圧縮・プリフェッチで制御する。巨大モデルは領域分割とメタタイル設計が必要。
    - 運用自動化：ファイル登録→変換→検証→デプロイのパイプラインをCI化し、差分更新（インクリメンタル）対応を実装する。

---

## 4.7 2D表示方針（3D統合）：XYZ/WMTS ドレープと MVT（ベクトルタイル）での高速表示

- **目的**: 3D（`3DTILES`）表示と統合された形で、背景ラスタ（オルソ/航空写真/DEM）を高速にドレープし、最小限の属性でインタラクティブなベクトル描画を行う運用方針を示す。
- **主要フロー（2D表示視点）**:
    1. 取り込み：GeoTIFF（オルソ/DSM/DEM）、ラスタモザイク、ベクタ（Shapefile/GeoJSON/PostGIS）を受け入れる
    2. 前処理：投影（EPSG）整合、リプロジェクト、ラスタのタイル化（`gdalwarp`→`gdal2tiles` / `gdal_translate`→`mbtiles`）、ベクトルのタイル化（`tippecanoe` / `ogr2ogr`→MVT）
    3. 配信：ラスタは`XYZ`/`WMTS`（Tileserver / CDN）、ベクトルは`MVT`（PBF）で配信、キャッシュ制御を付与
    4. 表示：3Dビューと同期してドレープ（Imagery/DEM レイヤ）し、ベクタはインタラクション（選択・属性表示・ハイライト）に利用

- **実装上の推奨**:
    - ラスタ: `WMTS`/`XYZ` で事前タイル化（タイルズームは表示用途に応じて決定、例: 背景 0–18）
    - ベクトル: `MVT`（Tippecanoe で生成）で属性は表示に最小限必要なもののみ（`ssot_id`, `type`, `label`, `status` など）に絞る
    - DEM: 高さ参照は 3D と整合させる（ジオイド/楕円体の基準を明記）、粗解像度は遠景用・詳細はローカル領域用に分割
    - タイル最適化: ジオメトリ簡略化・ジオメトリ圧縮・属性の省略でベクトルペイロードを削減
    - スタイル: サーバ側でスタイルを管理（Mapbox Style / TileServer GL）し、クライアントは最小のレンダリングロジックで描画

- **パフォーマンス対策（総合）**:
    - 同一タイル座標系を3Dと共有してキャッシュ効率を高める（可能なら WebMercator の共有グリッド）
    - CDN と長めの Cache-Control を利用しエッジキャッシュを最大化
    - タイル単位でプリフェッチ（視野予測）とレート制御を実装する
    - ベクトルは属性の遅延取得（軽量ジオメトリのみ先行、詳細属性は別APIで取得）を採用
    - クライアント側は WebGL レイヤ（Cesium/Deck.gl/MapLibre GL）で GPU 描画を行い、CPU 作業はワーカで分離
    - タイル生成は mbtiles（単一ファイル）でビルド→配信サーバで展開、差分更新はインクリメンタルで行う

- **運用上の留意点**:
    - 座標系・高さ基準は必ずメタデータ化して派生物に保存する
    - 公開用（層3）は解像度・属性を落とした派生物のみを配信する
    - ベクトルクエリ（属性検索）は PostGIS 等のサーバ側で実行し、クライアント負荷を下げる

- **推奨スタック例**:
    - ラスタタイル生成: `gdalwarp`, `gdal_translate`, `gdal2tiles` → `tileserver-gl` / `Caddy` + CDN
    - ベクトルタイル生成: `tippecanoe`（GeoJSON→MVT） / PostGIS→`tegola` または `TileServer` で配信
    - 表示: `Cesium`（3D） + `MapLibre GL` / `deck.gl`（2D/ベクトル）を組合せ、レイヤ同期でドレープ表示

---

## 4.8 2D表示方針（OGC準拠 — 推奨フォーマットと実装指針）

- **目的**: 2Dの地図・属性データ配信はOGC標準に準拠して相互運用性を確保する。3D表示（`3DTILES`）と整合した座標系・メタデータを保持しつつ、Web公開・関係者共有・編集ワークフローに対応する。

- **3D 連携**:
    - 2D 派生物は 3D 表示と直接ドレープ・同期できることを要件とする。背景ラスタ（XYZ/WMTS）・DEM は 3D クライアントの地形ドレープに使用可能であること。
    - 同一タイル座標系（可能なら EPSG:3857 / WebMercator）を採用し、タイル格子・ズームレベルの整合を保つ。タイル行列（TileMatrix）を 3D と共有することでエッジキャッシュとプリフェッチを共用できる。
    - DEM／高さ基準はメタデータで明示（`height_reference`: `geoid` | `ellipsoid` 等）。DEM の解像度とジオリファレンス精度が 3D 表示要件を満たすことを検証する。
    - 2D 側（OGC API - Tiles / WMTS / XYZ）と 3D 側（Cesium 等）のクライアント設定例をドキュメント化し、テストケースでピクセル整合性・位置誤差を検証する。
    - ベクトル（MVT）と 3D（3DTILES）の ID 連携: `ssot_id` を共通キーとして属性リンクを保持し、3D のオブジェクトと 2D の属性を双方向に参照可能にする。
    - ドレープワークフローの自動化: DEM / オルソ生成 → タイル化（WMTS/XYZ）→ メタデータ付与 → 配信 の CI を整備する。


- **推奨OGCプロトコル / フォーマット（優先順）**
    - **OGC API - Features（旧 WFS / REST ベース）**: 属性フィーチャの取得・検索・更新に推奨。JSON (GeoJSON) を標準ペイロードとして扱う。
    - **OGC API - Tiles / MVT（Mapbox Vector Tile）**: 高速ベクトルタイル配信。クライアントの描画性能を最大化するため、MVT（PBF）を採用する。
    - **WMS（OGC Web Map Service）**: 動的ラスタ画像（GetMap）や GetFeatureInfo 連携が必要な場合に利用。レガシーツール互換のため残す。
    - **WMTS（OGC Web Map Tile Service） / XYZ (TMS 相当)**: タイル化されたラスタ背景（航空写真・オルソ）を高速配信する標準手段。
    - **GeoPackage（OGC 〜 SQL ベースのパッケージ）**: 配布用／オフライン配布・アーカイブに推奨。ベクタ／ラスタを単一ファイルで保持でき、メタデータも内包できる。
    - **GeoJSON（OGC 相互運用的・軽量）**: API レスポンス、デバッグ、軽量クライアント向けのフォーマット。
    - **CSW / OGC API - Records（カタログ）**: メタデータ検索・カタログ連携に推奨。ISO 19115 準拠メタデータを格納。
    - **OGC API - Styles / Mapbox GL Style**: サーバ側スタイル管理とクライアント側の一貫した表現のために利用。

- **座標系・高さ基準**
    - 派生物（タイル・GeoJSON・GeoPackage 等）には必ず **EPSG コード** を明記する（例: EPSG:4326 / EPSG:3857 / プロジェクト固有CRS）。
    - 3Dと整合させるため、DEM/高さ参照（ジオイド or 楕円体）をメタデータに含める。

- **メタデータ（必須項目）**
    - `ssot_id`（UUID）、`title`、`created_at`、`creator`、`epsg`、`bbox`、`extent`、`accuracy`、`license`、`file_url`、`processing_history`。
    - カタログ登録時は ISO 19115 準拠フィールドを含め、OGC API - Records / CSW で検索可能にする。

- **配信・実装スタック（推奨例）**
    - **バックエンド DB**: `PostGIS` を主データソースとする。ジオメトリ・属性・バージョン管理を一元化。
    - **OGC サーバ**: `GeoServer` / `MapServer` / `QGIS Server(LIZMAP)`（WMS/WMTS/WFS/OGC API - Features）
    - **タイル生成**: `tippecanoe`（ベクトル）・`gdalwarp`/`gdal_translate`/`gdal2tiles`（ラスタ）・`mbtiles` 管理→CDN 配信
    - **ベクトルタイル配信**: `tegola` / `TileServer GL` / S3 + CloudFront（OGC API - Tiles 対応）
    - **クライアント表示**: `MapLibre GL` / `Leaflet`（軽量） / `deck.gl`（可視化/解析）／`Cesium` と同期（3D/2D レイヤ連携）

- **セキュリティ・アクセス制御**
    - 認証は OIDC/SAML を基本とし、API は OAuth2 トークン／API キーで制御。層2（タイプA）向けに短期署名付き URL を発行。
    - WMS/WFS 等のエンドポイントはリバースプロキシで保護し、必要に応じてレスポンスの部分的マスキング（公開用）を実装。

- **運用ルール・パイプライン**
    - ファイル登録→ETL（投影・簡略化）→タイル生成→検証（ジオリファレンス・属性整合）→デプロイ の CI パイプラインを構築する。CI には自動差分検出・インクリメンタル更新を組み込む。
    - 公開層（層3）は属性を削減し、個人情報が含まれるレイヤは必ず匿名化・集約した派生物を配信する。
    - ベクトルクエリは PostGIS 側で処理し、クライアントには軽量ID／範囲を返し詳細属性は別APIでフェッチさせる（遅延ロード）。

- **検証項目（PoC/導入時チェック）**
    - WMS/WMTS/OGC API - Features のレスポンス互換性と GetFeatureInfo の整合性検証
    - MVT（PBF）タイルの生成・ズーム整合性・スタイル適用テスト
    - GeoPackage の作成・読み戻しおよびオフライン利用検証
    - カタログ（OGC API - Records / CSW）での検索・メタデータ表示確認
