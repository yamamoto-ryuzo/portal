...
- **推奨方針（概要）**:
    - CDEはあくまでマスター（SSOT）を保持し、Webは参照・説明用の派生物配信に特化する。承認フローは派生データの公開前に Published（CDE外公開）ステータスで確定させる。
...
## 可視化の対象層（3パターン）と運用上の要件

プロジェクトにおける可視化は表面上は「見せる」機能に見えるが、実務では権限・粒度・承認フローで明確に役割を分離するべきである。以下の3パターンは運用設計の基準となる。

1) 設計担当者等の限定利用（内部高度利用）
- データ: 高忠実度の設計データ（IFC/DWG/原寸点群）、編集可能なソース
- ツール: Revit / Navisworks / デスクトップBIMクライアント等
- 要件: 書込・編集権限の限定、差分管理・ロック運用、作業用WIPの明確化

2) 関係者を含む広域参照（プロジェクト内共有）
- データ: 中〜高詳細の参照データ（3DTILES・軽量点群・注記付きビュー）
- ツール: ブラウザ可視化（Cesium / KOLC+ / CIMPHONY 等）に注記・承認機能を付与
- 要件: ロールベースのアクセス制御、操作ログ・承認ワークフロー、性能SLAの設定

3) 住民など一般公開向け（広報・合意形成）
- データ: 低詳細の説明用（glTF/スナップ画像/解説マップ・図解）
- ツール: 公開ポータル（Re:Earth 等のストーリーテリング機能）、SNS向け素材
- 要件: 個人情報・位置特定情報の匿名化、公開ポリシーと差し替えルール

共通運用ルール（簡潔）
- マスターはCDE内のSSOT（IFC/LAS 等）に限定し、可視化用は派生データとして自動生成して `Published` 承認を経て公開する。
- SaaS採用時はエクスポート・API・バックアップ方針を明確化し、ベンダーロックイン対策を講じる。
- 公開層ではプライバシー処理（マスキング・ぼかし）と公開承認プロセスを必須にする。

### 可視化層ごとのセキュリティ対策
以下は可視化の3パターン（設計担当者・関係者広域参照・住民公開）それぞれに推奨される具体的なセキュリティ対策である。

- **1. 設計担当者等（限定利用）**
    - 認証: SSO と多要素認証（MFA）を必須化し、作業端末をデバイス登録で限定する。
    - ネットワーク: VPN やプライベートエンドポイント等でアクセスを限定し、外部から直接アクセスさせない。
    - 権限: 最小権限モデルを適用し、編集権限はロール毎に厳格に管理する（WIP/ロック運用）。
    - データ保護: 保存時は暗号化（at-rest）・転送時はTLS、鍵管理はKMS/BYOKを採用する。
    - エクスポート制御: 大量エクスポート・外部ダウンロードは承認フロー・追加認証を必要とする。
    - 監査: 変更履歴・差分ログを不変に記録（ハッシュ/署名）、EDR とログ監視を併用する。

- **2. 関係者を含む広域参照（プロジェクト内共有）**
    - 認証/認可: RBAC による閲覧・注記・承認権限分離、短期署名付きURLは期限短縮。
    - セッション管理: セッションタイムアウト、IP制限や位置情報に基づく制約（必要時）を導入。
    - ログ連携: すべての操作ログをSIEMへ送信し監査・アラート設定を行う。ログ保管期間を定める。
    - 配信保護: TLS、WAF、CDN のセキュリティ設定、レート制限でDoS対策を行う。
    - 権限同期: CDE と可視化サービスのユーザ管理をSCIM/SSOで同期し、一元管理する。
    - バックアップ: 派生データ・メタデータの定期バックアップと復元手順を検証する。

- **3. 住民・一般公開（広報向け）**
    - データ匿名化: 位置解像度の粗度化、顔や車両等の検出・ぼかし・削除処理を自動化する。
    - 公開データ限定: 元データは非公開とし、公開用に生成した低解像度派生物のみを提供する。
    - 公開ポリシー: プライバシーポリシーとコンテンツ削除・訂正の手順を明示する。
    - 配信対策: CDN＋WAF、スクレイピング検知・IPレート制限を適用する。
    - ダウンロード制御: 公開ページでのダウンロード制限（静止画のみ許可等）を実施する。
    - 監視対応: 公開中のコンテンツをモニタリングし、問題発覚時は迅速に差し替え・撤回できる運用を整備する。

上記は初期推奨であり、導入するプラットフォーム・発注者要件に応じて細化・強化すること。

### 可視化層 × BOX共有モード × アプリ対応（簡易マッピング）
以下は『可視化層』ごとに「推奨するBOX共有モード」と「現実的に使いやすい可視化アプリ例」を対応付けた簡易マップである。導入時は各製品の認証・データ所在・API可否を確認のこと。

- **層1: 設計担当者（限定利用）**
    - 推奨BOXモード: コラボレータ（IDあり）
    - 推奨アプリ: Autodesk (Revit/Construction Cloud), Bentley iTwin, Trimble Connect, Navisworks, ArcGIS Pro, QGIS（デスクトップ）
    - 理由/注意点: 高忠実度ファイルは編集・差分管理が必要。Boxのコラボレータで操作ログ・編集権限を担保し、アプリ側はSSO/OAuth連携でユーザ同期する。

- **層2: 関係者広域参照（プロジェクト内共有）**
    - 推奨BOXモード: コラボレータ（公式共有）または共有リンク（タイプA: ID＋パスワード）
    - 推奨アプリ: Cesium / KOLC＋ / CIMPHONY / Re:Earth / iTowns / Potree（認証層を追加） / ArcGIS Online / ArcGIS Enterprise（ポータル） / QGIS Server / Lizmap
    - 理由/注意点: 関係者の識別・承認ログが必須。タイプA共有リンクは短期・限定共有で有用。可視化はブラウザ中心のためSSO/SCIM連携が重要。

- **層3: 住民・一般公開（広報）**
    - 推奨BOXモード: 共有リンク（タイプB：何もなし＝公開）または共有リンク（タイプA：ID＋パスワード）を併用
    - 推奨アプリ: Re:Earth（ストーリーテリング）、Cesium（公開ビューワ）、torinome（軽量配信）、glTF ビューワー、Lizmap、ArcGIS Online
    - 理由/注意点: 公開性を優先するためタイプBが使いやすいが、個人情報流出リスク低減のため派生物の匿名化を徹底。高リスク情報はタイプAで限定配布。


短い運用ルール:
- まずは層別ポリシー（どの層にどのデータを置くか）を承認し、Box共有設定と可視化サービスの認証連携（SSO/SCIM）を合わせて設計する。
- 可視化アプリの選定では「認証方式」「APIでの自動更新」「国内DC対応」を必須チェック項目とする。

## 役割別：工事関係者の可視化層割当（例）
以下は工事プロジェクトでの現実的な役割ごとの可視化層・BOX共有モード・推奨アプリの例示です。プロジェクト規模・機密度に応じて調整してください。

| 役割 | 標準的な可視化層 | 推奨BOX共有モード | 推奨アプリ・備考 | アカウント要否（無料で可） | KO条件（国内DC/ISMAP等） |
|---|---|---|---|---:|---|
| 施工管理者（現場監督） | 層2（関係者広域参照） | コラボレータ（IDあり） / 必要時タイプA | Trimble Connect, KOLC＋, Cesium（進捗ビュー）, QGIS/Lizmap（監理用マップ） — 進捗・工程確認、承認ワークフロー利用 | 要（組織アカウント、無償枠はサービス次第） | 要確認（官公庁案件は国内DC/ISMAP要件） |
| 現場技術者（測量・出来形担当） | 主に層1（限定利用）＋層2併用 | コラボレータ（IDあり） | Potree（点群確認）、Navisworks, Revit, QGIS（点群比較） — 高忠実度ファイルの参照/計測が必要な場合は層1権限 | 要（多くは有料ライセンス） | 要確認（取り扱いデータにより国内保管要件） |
| 下請け業者 | 層2（作業指示／参照） | 共有リンク（タイプA：ID＋パスワード）またはコラボレータ | Cesium / KOLC＋ / Re:Earth — 作業範囲・指示資料の参照。編集は限定的に付与 | 要（短期はリンクで無アカウント可能） | 確認要（データ機密性により国内DC要件） |
| BIMコーディネータ / モデラー | 層1（設計担当者） | コラボレータ（IDあり） | Revit / Navisworks / Bentley iTwin — モデル調整・差分管理を行うため層1が標準 | 要（有料ライセンスが一般） | 要確認（ソースデータ保管場所／可搬性） |
| 品質管理・監理（第三者） | 層2（承認・検査） | コラボレータ（IDあり）またはタイプA | QGIS/Lizmap（台帳・検査箇所）、Cesium（3D確認） — 証跡・監査ログが必要 | 要（組織アカウント） | 要確認（監査証跡保存要件） |
| 発注者・行政（窓口担当） | 層2（公式参照）＋層3（広報用抜粋） | コラボレータ（公式権限）／公開はタイプBで抜粋提供 | ArcGIS Online（ダッシュボード）、Re:Earth（説明用ストーリー） — 公的報告向けのビューを用意 | 要（多くはアカウント提供） | KO: 多くの場合 国内DC/ISMAP等の遵守が必須 |
...
### BOX の共有モード（コラボレータ vs 共有リンク）と推奨運用
Box を可視化ワークフローで利用する場合、共有モードは大きく2種類に分かれる。

- **コラボレータ（IDあり）**
    - 概要: 相手にBoxアカウントを付与してコラボレータ化する方式。アクセスはアカウント単位で認証・監査可能。
...
推奨運用:
- 設計／関係者共有（層1/2）は原則コラボレータで、共有リンクは副次的に低リスク情報のみ許可する。
