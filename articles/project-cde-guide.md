...
目的:
- 事業監理の意思決定に迅速に資する「概算事業費」を、過去の積算データをRAG化してAI（レトリーバ＋LLM）で参照・整理し概算を導出する運用とする。数量抽出や高精度な算出が必要な局面では GAIA 等の専門ツールを運用ルールとして位置付け、ヒューマンインザループで検証・調整を行う。入札用の精密積算とは区別して運用する。

必須インプット:
- SSOT の空間データ・支障物件台帳・既設情報
- 単価表（地域・年月版）と歩掛り・係数
- 事業監理が定義する前提（範囲・仕様・精度目標）

基本ワークフロー（短縮版）:
1. 事業監理が**前提定義**（対象範囲・精度目標・使用単価版）を確定する。
2. AI支援がCDE/SSOTから必要データを抽出し品質チェックを実施する（座標系・欠損・重複の確認）。
3. RAG化された過去の積算データを参照してLLMにより初期概算を生成する。必要に応じて空間データからの数量抽出は GAIA 等の専門ツールで実施し、その結果をRAG出力と突合して精度向上ルール（検証手順・係数適用）に従って調整する。
4. 単価テンプレートを適用して概算を算出し、内訳（工種別・概算率）を生成する。
5. 出力に**前提メタ（入力者・日付・単価版・使用データID・想定誤差）**と出典トレースを付与して事業監理へ提出。
6. 事業監理がレビュー・承認し、必要なら追加調査や係数調整を指示する。

精度管理と留意点:
- 目標精度を事前に定義（例: ±15%）し、過去案件の比較で係数を調整する。データ欠落や既設不明は誤差要因として明示する。
- 概算は意思決定用の参考値であることを冒頭に明記し、入札用の正式積算とは扱いを分離する。
