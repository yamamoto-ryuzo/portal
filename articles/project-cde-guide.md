...
- `Exchange` で受領したファイルは速やかに検査・分類し、重要なものは `Shared`／`Published` 等の正式フォルダへ移行する。業務フォルダ（WIP/Shared/Published/Archived）はコラボレータ限定の厳格なアクセス制御を維持する。
...
- 移行期限：`Exchange` 内の重要ファイルは原則 72 時間以内に分類・Box もしくは該当フォルダへ移行する。
...
`Exchange` は CDE 外で行われたメール添付やチャットでのやり取りを受け止めるバッファであり、あくまでコピーの一時保管場所です。受領後は TaskTeam で検査・整理を行い、整理済みのファイルを TaskTeam が `WIP` に提出して正本（SSOT）に取り込みます。短期的な受領→整理→提出のワークフローをフロー図に反映しています。
