...
4. [GIS基盤・空間データ可視化基盤](#4-gis基盤空間データ可視化基盤)（GIS：空間データの管理・配信・2D/3D可視化）
5. [デジタルツイン活用層](#5-デジタルツイン活用層)（BIM/IFC × 点群 × IoT × GISの統合・リアルタイム同期・シミュレーション）
6. [承認フロー（ワークフロー）](#6-承認フローワークフロー)
7. [タスクチーム作業環境](#7-タスクチーム作業環境)
8. [付録](#8-付録)
...
| 第1層 | 蓄積・保管層（業務データ基盤 / RAG支援基盤） | SSOT保管・RAG基盤 | 文書・図面・BIM/IFC・点群等をSSOTで保管し、RAG向けのインデクシング・ベクトルDB・メタデータ管理を備え、AIチャットボットや要約・生成支援を支える基盤を提供する。 |

### 第4層（AI活用層）— 具体的ユースケースとワークフロー
- 予測分析: 工程遅延予測（時系列モデル）、コスト超過リスクの早期検知、資材需給予測。
- 自動要約・レポート生成: 日報・報告書の要約、会議議事録の自動生成、承認判定の要約提示。
- インテリジェント検索（RAG）: 図面・仕様・契約書からの質疑応答、過去類似事例の抽出。
- アラート・異常検知: センサー監視による閾値超過通知、点群差分による出来形異常検出。
- ワークフロー連携: チケット起票・差戻し理由の自動分類、承認者への要点提示。

ワークフロー概略: データ収集 → インデックス（メタデータ/ベクトル）→ 検索/レトリーバ → LLM再構成（プロンプトテンプレート）→ 結果配信（ダッシュボード/通知）

### 第1層（RAG支援基盤）— 推奨構成と注意点
- コア: ベクトルDB（例: Qdrant / Milvus / Weaviate / Pinecone）＋メタデータストア（Postgres等）。
- 埋め込み: 高品質な埋め込みモデル（OpenAI embeddings / Instruction-tuned local models / SentenceTransformers）。
- レトリーバ: フィルタリング（メタデータ条件）＋再ランキング（BM25や学習ベース）。
- LLM連携: システム指示＋参照文献提示（出典付き回答）＋冪等性考慮。
- セキュリティ: PIIフィルタ、アクセス制御、ログ・監査、オンプレミス/国内データセンタ対応の検討。
- 運用: データ更新時の再インデックス化ポリシー、コスト試算（ベクトルDB・呼び出しコスト）、応答品質評価指標（正解率・再現率・Hallucination率）。

**第1層：業務データ基盤**（ストレージ）
: データ取り込み（バッチ／ストリーム）、ETL/ELT、スキーマ・メタデータ管理、アクセス制御・監査、バックアップ、API公開、データガバナンス等を担い、SSOT（属性・履歴・トランザクション）を保持する。

**第2層：GIS基盤・空間データ可視化基盤**
: 地理空間データ（ベクタ・ラスタ・点群・BIM）の統合・配信・2D/3D可視化を担う。PostGIS等の空間DBを中核に、OGC標準（WMS/WFS/3DTILES/MVT）でデータを配信し、関係者が地図・3D・グラフで状況を把握できる基盤。空間的な意思決定支援・台帳管理が主目的。

**第3層：デジタルツイン活用層**
: GIS基盤・BIM/IFC・点群・IoTセンサーデータを統合し、現実の物理対象（構造物・インフラ・都市）をデジタル空間上にリアルタイムで再現・同期する。施工進捗の状態監視・シミュレーション・将来予測・VR/AR活用が主目的。GIS基盤を「データ供給源」として活用し、さらに上位の意思決定・運用管理を実現する。

**第4層：事業監理業務**
: 第1〜3層のデータを活用して進捗管理・資料整理・可視化・報告を行う。発注者・受注者・関係機関が横断的に事業状況を把握し、意思決定を行うマネジメント層。ダッシュボード・レポート・会議資料はすべてSSOT（第1層）から派生させ整合性を担保する。

**第5層：SSOTによる共有・公開**（目的層）
: 承認済みデータ（Publishedステータス）を関係機関・外部関係者・市民へ確実に届けるガバナンス層。公開用の派生物は必ずSSOTから生成・配信し、出所不明なデータの流通を防ぐ。プロジェクトCDE全体の最終目的はこの層の実現にある。
...
**表4-7　データ利用方式の全体対応表（層・方式・共有方法・アプリ）**
...
**表5-5　DTプラットフォーム比較（KOLC+ / Re:Earth / torinome）**
...
## 5.4 代表事例: torinome（XRデジタルツインPF・HoloLab）
...
## 5.5 KOLC+（建設BIM/CIMデジタルツインプラットフォーム）
...
- **広域表示の限界**: 都市・地域スケールの広域高速表示・PLATEAU連携・住民説明向け公開可視化は Re:Earth（5.6）が適している。KOLC+ は工事現場〜路線スケールに特化しており、広域地図的な用途は本来の守備範囲外。
...
## 5.6 Re:Earth（スマートシティ・都市デジタルツインプラットフォーム）
...
- **BIM/CIM詳細管理の限界**: 工事現場スケールのBIM/CIM詳細管理・出来形・4D工程・リアルタイム同期は KOLC+（5.5）が適している。Re:Earth は「広く見せる」可視化層であり、「深く管理する」施工管理DTとは用途が異なる。
...
## 6.1 CDEの決済フロー（WIP → Shared → Published / Archived）
...
## 6.4 Webhook起点での実装方式（比較）

**表5-4　Webhook起点での実装方式の比較**
...
**表6-4　〔A-BIM〕BIM/CIM同時並行編集 候補・総合評価**
...
### torinome（デジタルツイン活用層 代表事例）

> 詳細は **[5.4 代表事例: torinome（XRデジタルツインPF・HoloLab）](#54-代表事例-torinomexrデジタルツインpfhololab)** を参照。

## 8.2 認証・API・国内DC 暫定まとめ

> 下表は公開情報を基にした暫定評価。公共案件でのKO判定（国内DC必須等）を想定する場合、各ベンダーの**官公庁向けプラン／ガバメントクラウド対応／契約書面での所在地明記**を必ず取得すること。

**表8-2　認証・API・国内DC 暫定まとめ**
