...
flowchart TD
    ...
    ASP["ASP（KOLC＋ / Basepage 等）<br/>受発注者間のASP承認ワークフロー"]
...
**補足 — ASP（KOLC＋/Basepage 等）から `Archived` へ移行するルート**

- KOLC＋/Basepage 等の ASP 上で承認ワークフロー（Shared→Published 相当）を完了した成果物は、API または Webhook を用いて自動的に CDE の `Archived`へ最終版を登録できるルートを設けることを推奨する。
- 実務フロー: ASP 側での承認完了 → `Published` 登録（SSOT ID 付与）→ Webhook/API による `Archived` への最終版登録（監査ログ・承認メタデータを付与）
- 技術要件: `ssot_id` とファイルIDの紐付け、承認者・承認日時・差戻し履歴を含む監査ログの保存、登録時の冪等性（重複登録防止）設計。
- 運用留意: ネットワーク不通時のフォールバック（ローカルキュー／手動アップロード）を定義し、復帰後に自動同期する手順を明記する。
