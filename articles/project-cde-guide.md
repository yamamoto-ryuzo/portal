...
GIS基盤（第4章）= データインフラ
  │
  ├── データを供給 → デジタルツイン活用層（本章）
  │       ・ PostGIS / GeoServer / TileServer GL
  │       ・ 空間データの保管・配信・2D/3D可視化
  │       ・ CesiumJS（3D描画エンジン）/ MapLibre GL（2D描画エンジン）
  │
  └── 活用層（デジタルツイン）= アプリケーションプラットフォーム
          ・ 建設BIM/CIM DT: KOLC+（4D工程・出来形・現場リアルタイム同期）
          ・ 都市DT: Re:Earth / PLATEAU VIEW（スマートシティ・住民説明会）
          ・ XR DT: torinome（AR/VR/WebAR）
...
| 観点 | GIS基盤 | デジタルツイン活用層 |
|------|---------|------------------|
| **本費** | 地理空間データの管理・分析・可視化の基盤 | 現実の物理対象をデジタルで再現・同期する概念 |
