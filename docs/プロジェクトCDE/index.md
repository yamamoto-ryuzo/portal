...
|----|--------|----------|-----------|-----------|------------|
| **層1** 設計・編集 | 設計者・BIM担当・測量担当 | アカウント（認証付き） | **ネイティブ方式** 原本ネイティブ | 🗺️ **GIS基盤** | `AutoCAD`/`Civil 3D`・`Revit`・`Navisworks`・`ArcGIS Pro`・`QGIS`・`Leica Cyclone`（点群） |
| 〃 | 〃 | 〃 | 〃 | 🏗️ **デジタルツインPF** | `KOLC+`（建設BIM/CIM DT → **5.4**） |
| **層2** 関係者共有 | 発注者・施工管理・関係機関 | アカウント ＋ 限定共有（署名付き短期URL等） | **ネイティブ方式 or タイル配信** | 🗺️ **GIS基盤** | ネイティブ方式: `Trimble Connect`・`BIM 360 Viewer` ／ タイル配信: `Lizmap`（認証付き）・`CesiumJS`（3D）・`MapLibre GL`（2D） |
...
> 補足: ネイティブデータは必要に応じてタイルデータへ変換し、配信・表示に利用するワークフローも可能です。
...
**原本（ネイティブ方式）は CDE で SSOT として保管・管理し、共有・公開・AR 用にはタイル配信方式の派生物を自動生成する。**
