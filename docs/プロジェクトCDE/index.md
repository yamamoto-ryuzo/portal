...
- 協議（外部向け）は `03_.../Published` まで到達させる
- 内部会議記録は横断フォルダ `00_会議・議事録` に集約する
- 実データ（正本）は `06_External_SSOT` に置き、CDE には必ずマニフェストを置く
...
📁 [事業名]-プロジェクトCDE
│
├── 📁 00_Exchange【やり取り】   ← CDE外受領の一時バッファ（速やかに分類）
├── 📁 01_TaskTeam【タスクチーム】 ← 各チームの作業領域（チーム内制限）
├── 📁 02_WIP【作業中】            ← 当該チームの作業WIP
├── 📁 03_Shared【共有】           ← プロジェクト関係者全員が閲覧
├── 📁 04_Published【活用】       ← 承認済み現行版（外部配布・運用で使用）
├── 📁 05_Archived【アーカイブ】  ← 変更不可・成果物保管
└── 📁 06_External_SSOT【外部（SSOT）】 ← 実データ正本（外部ストレージ）
```

### `06_External_SSOT` の具体構成（推奨）

- `01_AD/`       — 元文書・OCR・前処理テキスト・変換ログ
- `02_GIS/`      — GeoTIFF, タイル, PostGIS バックアップ, メタデータ
- `03_DigitalTwin/` — 3DTILES, BIM 同期データ, 時系列センサデータ
- `04_RAG/`      — 前処理テキストのマニフェスト（埋め込み本体は専用ベクタDBへ）
- `05_API/`      — 提供 API 定義・エンドポイント情報
- `manifests/`   — `index.csv`（横断索引）・`published_manifest_<案件>.yaml` 等

### マニフェスト（CDE 側） — 最低限の必須項目

- `ssot_id` — 正本の一意識別子
- `consumer` — CAD/GIS/DT/RAG 等のカテゴリ
- `s3_path` / `db_uri` — 正本の実体参照
- `checksum` — 整合性確認用
- `version` — バージョン
- `published_at` — 公開日時
- `owner` — 所有部署／担当者
- `access_policy` — アクセス制御情報（署名付きURL TTL 等）
...
- マニフェストは検索性と監査の鍵。`ssot_id` と `consumer` を必須にすること
- `06_External_SSOT/manifests/index.csv` で横断索引を作り、`00_会議・議事録` などからのリンク運用を推奨
...
│  
└── 📁 06_External_SSOT【外部（SSOT）】   ← 実データはS3等に配置。Box には必ずマニフェストを置く
    ├── 📁 01_AD【文書／前処理】        ← 元文書、OCR・前処理テキスト、変換ログ等
    ...
    └── 📁 manifests                  ← `index.csv` や `published_manifest_<案件>.yaml` を配置
```
...
## 2.7 外部SSOT運用（データ種別ごとの簡潔マッピング）

目的: Box の CDE と別に「外部システム向けの正本（SSOT）」をどのデータをどこに保存するか、短く明示する。

保存先の凡例:
- `Box CDE`：レビュー・ドキュメント原本（小容量）・マニフェスト・プレビュー・手順書
- `External SSOT`：S3 等のオブジェクトストレージ、PostGIS、専用ベクタDB（埋め込み／インデックス）、時系列DB

データ種別ごとの推奨保存先（簡潔）:
- 高解像度ラスタ（GeoTIFF / DSM / DTM） → External SSOT（S3）
    - 理由: 容量大・配信タイル化が必要。Box はプレビューのみ。
- タイルセット（MBTiles / 3DTILES フルセット） → External SSOT（S3 / タイルサーバ）
    - 理由: 高速配信・CDN 経由が必要。Box はマニフェスト/サムネのみ。
- 点群（LAS/LAZ / Entwine タイル） → External SSOT（S3 / 点群タイルストア）
    - 理由: 巨大ファイル・タイル配信用。Box はサンプル/プレビューのみ。
- BIM フルソース（IFC / Revit 原本） → External SSOT（S3 または専用BIMストレージ）
    - 理由: 原本は正本として長期保管・大容量。Box には承認PDF/抜粋を置く。
- ベクトルデータ（大規模 GeoPackage / Shapefile / PostGIS） → External SSOT（PostGIS / S3 + DB バックアップ）
    - 理由: 検索・クエリ性能を要するため DB 管理。
- 時系列センサデータ（RAW） → External SSOT（時系列DB / オブジェクトストレージ）
    - 理由: 高書込頻度・長期保存・解析に最適化。
- 埋め込みベクトル・ベクタインデックス（RAG 用） → External SSOT（専用ベクタDB: Pinecone/Milvus/FAISS 等）
    - 理由: 高速検索・機密性の観点から専用管理を推奨。
- 元文書（PDF/図面の原本） → Box CDE（Published を正本として保管） + External SSOT にメタ（必要時）
    - 理由: ドキュメント管理・承認ワークフローは CDE が適任。大容量図面は S3 にも保持してマニフェストで整合。
- 前処理済テキスト（RAG 用抜粋） → Box CDE（確認用・マニフェスト） + External SSOT（全文/原文は外部）
    - 理由: Box は人がレビューする場所、埋め込み本体は専用 DB に置く。
- マニフェスト / メタデータ / README / data_contract.md → Box CDE（Published/metadata）
    - 理由: コラボ・承認・検索の利便性確保。
- サムネイル・低解像度プレビュー → Box CDE
    - 理由: レビュー用に軽量データを置く。

Exchange（やり取り）との違い（明快に）:
- `Exchange` は「一次受領・一時バッファ（CDE外）」であり、一時保管・検査・WIP への移行が目的。
- `外部SSOT` は「外部システムが参照する正式な正本（API/署名付き URL 等で提供）」であり、可用性・パフォーマンス・契約が要求される点で目的が異なる。

実務チェック（短い）:
- 外部公開・API で参照するデータは External SSOT に置き、`Box CDE` には必ずマニフェスト・承認情報を残すこと。
- 埋め込み／インデックスは Box に置かない。前処理テキストは Box に置いてレビュー可能にする。

この短いマッピングを `index.md` に反映しました。より簡潔な表（CSV/表形式）で別ファイル化しますか？
