...
| **AR・現場** | 施工・監理担当・住民説明会 | アカウント または 公開 | **方式B** 3DTILES/glTF | `torinome AR`・`torinome WebAR`・`Cesium for Unity/Unreal`・`PLATEAU AR` 等（**→ 詳細は第5章「デジタルツイン活用層」5.4参照**） |
...
## 4.11 方式B 代表事例: torinome

> **本節はXRデジタルツインプラットフォームとして第5章「デジタルツイン活用層」に移動しました。**  
> → [5.4 代表事例: torinome（XRデジタルツインPF・HoloLab）](#54-代表事例-torinomexrデジタルツインpfhololab)
...
> **本章の位置づけ**  
> デジタルツイン活用層は、「GIS基盤（第4章）が提供する空間データ」を活用して、現実の物理対象（構造物・インフラ・都市）をデジタル空間上に再現・同期する。GISが「手段」であるのに対し、デジタルツインは「目指す姿・活用層」である。
...
GIS基盤（第4章）
  ...
  │       ・ PostGIS / GeoServer / QGIS Server
  │       ・ 空間データの保管・配信・2D/3D可視化
  │
  └── 活用層（デジタルツイン）
          ・ BIM/IFC × 点群 × IoTの統合
          ・ Cesium / PLATEAU / torinomeでの3D可視化
          ・ 施工管理・維持管理での状態同期
          ・ シミュレーション・将来予測・VR/AR活用
```
...
| 構成要素 | 内容 | 主なツール例 |
|---------|------|------------------|
| **3D座標層** | BIM/IFC・点群・PLATEAUの統合・3DTILES配信 | Cesium, PLATEAU VIEW, torinome |
...
> **「KOLC+（方式A代表事例）」はGIS/BIM基盤ツールとして第4章（4.10）に記載する。XRデジタルツインプラットフォームの「torinome」は本章 5.4 に記載する。**

## 5.4 代表事例: torinome（XRデジタルツインPF・HoloLab）

ホロラボ（HoloLab Inc.）が開発・運営する XR デジタルツインプラットフォーム（サービスページ: https://hololab.co.jp/services/packages/torinome/）。GIS基盤（第4章）が提供する 3DTILES・LAS・GeoJSON 等を XR/AR/VR 環境に接続し、現実とデジタルを融合する**デジタルツイン活用層の代表事例**。方式B（タイル配信）の層2〜3・AR/VR/WebAR を単一プラットフォームで一気通貫カバーできる点が最大の特徴。
