...
プロジェクトCDEは5層構造で成立する。下位3層（第1〜3層）はデータの蓄積・統合・活用基盤を担い、上位2層（第4〜5層）はそのデータを用いて事業監理・公開を行うガバナンス層である。
...
| 第3層 | デジタルツイン活用層 | BIM/点群/IoT統合・シミュレーション | BIM/IFC・点群・都市モデルとIoTを統合し、リアルタイム同期やシミュレーションで現場の分析・検証を行う。 |
...
### 第4層（AI活用層） — 要点と主なユースケース
- 予測分析: 工程遅延・コスト超過・資材需給の予測
- 自動要約・レポート生成: 日報や議事録の要約、承認用サマリ作成
- インテリジェント検索（RAG）: 図面・仕様書・契約書からの問い合せ応答
- 異常検知・アラート: センサー／点群差分による異常の自動検知
- ワークフロー連携: チケット自動分類、承認者への要点提示

ワークフロー概略: データ収集 → インデックス（メタデータ／ベクトル）→ 検索／レトリーバ → LLM再構成（プロンプト）→ 結果配信（ダッシュボード／通知）

### 第1層（RAG支援基盤） — 推奨構成（要点）
- コア: ベクトルDB（例: Qdrant / Milvus / Weaviate / Pinecone）＋メタデータストア（Postgres等）
- 埋め込み: 高品質な埋め込みモデル（OpenAI embeddings / SentenceTransformers 等）
- レトリーバ: メタデータフィルタ＋再ランキング（BM25や学習ベース）
- LLM連携: 参照付き回答（出典提示）、プロンプト設計、冪等性の確保
- セキュリティ・運用: PIIフィルタ、アクセス制御、監査ログ、再インデックス化ポリシー、コスト試算、応答品質評価
...
**第1層：蓄積・保管層（業務データ基盤 / RAG支援基盤）**
: 目的: SSOT（原本）を確実に保持し、データ取り込み・ETL・メタデータ管理・API提供・アクセス制御を行う基盤。
- 主な機能: ファイル原本管理、バージョン管理、メタデータカタログ、バックアップ、監査ログ、データライフサイクル管理。
- RAG対応: 埋め込みパイプライン、ベクトルDB＋メタデータストア、再インデックス化ポリシー、品質評価（精度/再現/ハルシネーション監視）。
- 運用要点: 座標系とメタデータ必須化（ISO19115準拠推奨）、PIIフィルタ、アクセス監査、オンプレミス／国内DC検討。
