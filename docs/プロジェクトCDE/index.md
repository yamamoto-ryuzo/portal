...
└── 📁 06_External_SSOT【外部SSOT】   ← 実データはS3等に配置。Box には必ずマニフェストを置く
    ├── 📁 01_GIS【地理空間データ】     ← ラスタ（高解像度）、ベクタ、タイル、点群、タイル配信設定
    ├── 📁 02_DT【デジタルツイン】     ← 3Dタイル、BIM同期データ、時系列センサデータ
    └── 📁 03_RAG【検索／RAG用埋め込み】← 埋め込みベクトル、ベクタDB参照、原文テキストの索引
```
...
- `01_GIS/`      — GeoTIFF 等ラスタ、タイル、ベクトル、点群バックエンド（CRS/解像度等のメタを含む）
- `02_DT/`       — 3Dタイル、BIM 同期データ、時系列センサデータ（バージョン管理・同期ログ）
- `03_RAG/`      — 前処理テキストの索引用マニフェスト／埋め込み参照情報（埋め込み本体は専用ベクタDBへ）
...
以下は `06_External_SSOT/` の子フォルダ別の簡潔な説明と運用要件（GIS/DT/RAG に集約）。
...
共通の必須運用要件（要点）
- マニフェストは Box に置き、`ssot_id` / `consumer`（GIS/DT/RAG 等） / `s3_path`または`db_uri` / `version` / `owner` / `access_policy` を明記すること。
