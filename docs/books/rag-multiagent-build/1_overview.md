...
[土木事業管理 RAG_Sysyem構築ガイド](../rag-civil-engineering/1_intro.md)（以下「設計書」）では、  
...
>  
> RAG_Sysyemが「答えてしまう」のは検索が成功したからではなく、**LLMが検索ミスに気づかず回答するから**である。  
...
| 登録件数を増やせば回答が良くなる | 正しい文書が上位N件に入る設計になっているかが重要 |
...
多くのRAG_Sysyemでは、登録済み文書が1000件あっても、1回の回答でLLMに直接渡るのは上位数件〜十数件の**チャンク**であり、文書全体ではない。  
...
> **「モデルを変えても精度が上がらない」理由**  
> RAG_Sysyemの精度が低い場合、LLMモデルのアップグレードではなく上記 Step 1〜3 の順に確認することが効率的である。  
