---
title: "簡易実装アプローチ：MD＋AI指示による近似実現"
---
# 6. 簡易実装アプローチ：MD＋AI指示による近似実現

§4（7ステップ）はフル実装版である。PoC・小規模運用では、**4種のMDファイル＋システムプロンプト**で同等の効果を近似できる。**開発スタイル：ノーコード〜ローコード** — プログラミング不要で **M365 Copilot Agent Builder**・Dify・ChatGPT等のAIプラットフォームに設定するだけ。インフラ不要・構築期間は数日単位。

## 6.1 フル実装 vs 簡易実装の対応関係
...
## 6.3 entity_dictionary.md（Steps 2-4 代替）
...
## 6.6 knowledge_map.md（Step 5 代替）
...
## 6.8 system_prompt.md（Steps 6-7 代替）

このファイルをDify・n8n等のRAG_Sysyemの**システムプロンプト**として設定する。オントロジーの制約ルールとCogGRAGの分解手順をAIへの指示として記述する。
