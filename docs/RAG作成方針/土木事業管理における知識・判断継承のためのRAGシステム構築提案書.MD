...
> **主力推奨は Track A+C**。AIの推論能力向上・大コンテキスト化の恩恵を直接受けながら、MD管理者（ドメイン専門家）だけで維持できる。Track B が必要になる条件は「法的監査証跡の要求」「10万件超の大規模文書」「複数組織間のスキーマ共有」に限定される。
...
| フル実装の機能 | マルチエージェントでの近似 |
|---|---|
| CogGRAGの問題分解 | オーケストレーターエージェントが問いを分割して専門エージェントに委譲 |
| GraphRAGの多段推論 | 専門エージェントが順番に検索し、前エージェントの結果を引き継ぐ |
| SHACLの整合性検証 | 監理エージェントが全エージェントの回答を検証・矛盾を棄却 |
| オントロジーの正規化 | 全エージェントが共通の `entity_dictionary.md` を参照 |
...
- **Track A+C**: LLMのバージョンアップするだけで推論精度・整合性検証の品質が自動的に向上する。MDファイルとプロンプトは変更不要。
- **Track B**: インフラ（Graph DB・SPARQL・SHACL）の精度はLLM進化の恩恵を受けにくく、改善にはエンジニアリング工数が必要。
...
ユーザーからの問い合わせに対して、システムは「観察駆動型AIエージェント」として動作し、以下の自律的な思考サイクルで回答を生成する。
...
1. **観察（Observation）**: ユーザーの質問と現在の思考マップの状況を精査する。
2. **行動（Action）**: CogGRAGで問題を分解し、GraphRAGのグラフ探索とVectorRAGのベクトル検索を同時に実行して関連知識をロードする。
3. **反省・検証（Reflection）**:
   - **ノイズ削除**: コンテキストと矛盾する情報を排除する。
   - **整合性検証（Dual-Process）**: 中間回答が「法令トリプルのロジック（論理的整合性）」と「現場報告書テキスト（実務的現実性）」の双方から矛盾がないかを検証する。
   - **ループ制御**: 情報不足や2ホップ以上の論理的飛躍を検知した場合は、グラフの隣接ノードを拡張して再検索する。
